...
from src.model.assets_under_management import Commitment, Investor


async def get_investors() -> List[InvestorResponse]:
    session = Database.get_session()
    # use a subquery to calculate the total commitment for each investor
    subquery = session.query(Commitment.investor_code, func.sum(Commitment.amount).label("total_commitment")).group_by(Commitment.investor_code).subquery()
